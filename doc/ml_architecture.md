...
- Data loading is separate from preprocessing
- Training is separate from optimization
- Model persistence is centralized

### Reproducibility
- All preprocessing steps are saved with the model
- Hyperparameters and metrics are versioned
- Random seeds are controlled
- Training data characteristics are logged
...
### Versioning
- Models are versioned semantically (major.minor.patch)
- Preprocessing artifacts are saved with models
- Metadata tracks training date, data version, metrics

### Extensibility
- Easy to add new model types (e.g., neural networks)
- Easy to add new features to preprocessing pipeline
- Easy to swap out hyperparameter optimization strategies
...
- Industry standard for sklearn pipelines

### Why Separate Preprocessors?
- Same preprocessing must be applied to training and inference data
